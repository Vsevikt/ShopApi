    ...
    public class OrderProcessingService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    ...
                    Console.WriteLine(">>> ПОВІДОМЛЕННЯ УСПІШНО ОПРАЦЬОВАНЕ ТА ПІДТВЕРДЖЕНЕ (Ack) <<<");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"\n\n[КРИТИЧНА ПОМИЛКА У ФОНОВОМУ СЕРВІСІ]:\n{ex.Message}\n{ex.StackTrace}\n\n");
        ...
        private async Task ProcessOrderLogicAsync(OrderMessageDTO orderData)
        {
            ...
            if (hasEnoughStock)
            {
                ...
                {
                    emailBody += $"<li><b>{p.Name}</b>: {p.Count} шт. х {p.Price} грн. = {p.ItemTotalPrice} грн.</li>";
                }
                emailBody += $"</ul><h3>Загальна сума до сплати: {orderData.TotalPrice} грн.</h3>";
